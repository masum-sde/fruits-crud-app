...
app.get("/getSingleFruit/:id", (req: any, res: any) => {
  if (isValidToken(req)) {
    fruit.findOne({ where: req.params }).then((result: any) => {
      if (result) {
        res.send(result);
      } else {
        res.send("Not Found!!");
...
app.get("/getSingleFruit", (req: any, res: any) => {
  if (isValidToken(req)) {
    fruit.findOne({ where: req.query }).then((result: any) => {
      if (result) {
        res.send(result);
      } else {
        res.send("Not Found!!");
...
app.delete("/deleteSingleFruit/:id", (req: any, res: any) => {
  if (isValidToken(req)) {
    fruit.findOne({ where: req.params }).then((result: any) => {
      if (result) {
        fruit.destroy({ where: req.params }).then((data: number) => {
          if (data > 0) {
